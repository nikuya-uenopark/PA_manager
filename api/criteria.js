...
カテゴリ：${category || "共通"}`
      ...
      res
        .status(200)
        .json({ id: created.id, message: "評価項目が追加されました" });
      ...
      const items = Array.isArray(body.items)
        ? body.items.map((x, idx) => ({
            id: x.id,
            index: Number.isInteger(x.sortOrder) ? x.sortOrder : idx,
          }))
        : Array.isArray(body.order)
        ? body.order.map((x, idx) => ({
            id: x.id,
            index: Number.isInteger(x.sort_order) ? x.sort_order - 1 : idx,
          }))
        : null;
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        ...
        if (invalid)
          return res.status(400).json({ error: "invalid id in payload" });
...
件数：${ordered.length}`
      ...
      if (!id)
        return res
          .status(400)
          .json({ error: "id is required or provide order/items array" });
