...
状態：${label}`
      ...
      return res
        .status(200)
        .json({ id: created.id, message: "評価データが追加されました" });
    }
    ...
    if (req.method === "PUT") {
      ...
      if (result.count === 0) {
        await prisma.evaluation.create({
          data: { staffId: sid, criteriaId: cid, status: status || "learning" },
        });
      }
      const [staff, crit, changer] = await Promise.all([
        prisma.staff
          .findUnique({ where: { id: sid }, select: { name: true } })
          .catch(() => null),
        prisma.criteria
          .findUnique({ where: { id: cid }, select: { name: true } })
          .catch(() => null),
        changedBy
          ? prisma.staff
              .findUnique({
...
状態：${label}`
    ...
    if (req.method === "DELETE") {
      const { id } = req.query || {};
      const before = await prisma.evaluation
        .findUnique({
          where: { id: Number(id) },
          select: { staffId: true, criteriaId: true, status: true },
        })
        .catch(() => null);
...
id：${id}`
    ...
    return res
      .status(500)
      .json({ error: "評価API エラー", detail: error.message });
  }
};
