...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const items = changes
      ...
      .filter(
        (c) => Number.isInteger(c.staffId) && Number.isInteger(c.criteriaId)
      );
    ...
    await prisma.$transaction(async (tx) => {
      for (const it of items) {
        ...
        const target = (req.body.changes || []).find(
          (c) =>
            Number(c.staffId) === it.staffId &&
            Number(c.criteriaId) === it.criteriaId
        );
        if (target && target.test) {
          if (typeof target.test.testedBy === "number") {
            const comments = JSON.stringify({
              testedBy: Number(target.test.testedBy),
              testedAt: target.test.testedAt || new Date().toISOString(),
            });
    ...
    let changerName = "-";
    if (changedBy) {
      const changer = await prisma.staff
        .findUnique({
          where: { id: Number(changedBy) },
          select: { name: true },
        })
        .catch(() => null);
    ...
    const [staffListAll, criteriaListAll] = await Promise.all([
      prisma.staff
        .findMany({
          where: { id: { in: staffIdsAll } },
          select: { id: true, name: true },
        })
        .catch(() => []),
      prisma.criteria
        .findMany({
          where: { id: { in: criteriaIdsAll } },
          select: { id: true, name: true },
        })
        .catch(() => []),
    ]);
    const staffNames =
      staffListAll.map((s) => s.name || `#${s.id}`).join("、") || "-";
    const criteriaNames =
      criteriaListAll.map((c) => c.name || `#${c.id}`).join(", ") || "-";
    const logMessage = `評価更新\n変更者：${changerName}\nスタッフ：${staffNames}\n件数：${items.length}\n項目：${criteriaNames}`;
    await addLog("evaluation:batch-update", logMessage).catch(() => {});

    return res
      .status(200)
      .json({ message: "batch updated", count: items.length });
  } catch (e) {
    console.error("evaluations-batch error", e);
    return res
      .status(500)
      .json({ error: "evaluations-batch failed", detail: e.message });
  }
};
